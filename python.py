...
end_date = '2025-05-03'  # date.today().strftime('%Y-%m-%d')
...
def get_yfinance_data(symbol, start_date, end_date):
    ...
    if existing_start_date is None or existing_end_date is None:
        new_data_end_date = end_date + pd.Timedelta(days=1)
    ...
    # case 1: start date and end date are before existing range s---e___es===ee
        ...
        case = 1
        print(
            f'no existing data:  Case: {case}\n new date range: {symbol}, {date_ranges[0][0]}, {date_ranges[0][1] - pd.Timedelta(days=1)}')
    ...
    # case 6: start date is before existing range and end date is after existing range s---es===ee---e
        ...
        date_ranges.append(
            (existing_end_date + pd.Timedelta(days=1), new_data_end_date + pd.Timedelta(days=1)))  # [s, e]
        case = 6
        print(
            f'partial existing data:  Case: {case}\n new date range: {symbol}, {date_ranges[0][0]}, {date_ranges[0][1]}')
        print(
    ...
    try:
        for start_date, end_date in date_ranges:
            ...
            print(f'making data mask for {symbol} from {start_date} to {end_date - pd.Timedelta(days=1)}')
            date_range = pd.date_range(min(filter(pd.notnull, [start_date, existing_start_date])), max(filter(pd.notnull, [end_date - pd.Timedelta(days=1), existing_end_date])))
            ...
            print(f'getting data from yfinance for {symbol} from {start_date} to {end_date - pd.Timedelta(days=1)}')
            ...
            print(f'updating all_symbols_data for {symbol}: \
                  \n new date range: {new_data_start_date}, {new_data_end_date - pd.Timedelta(days=1)}\
